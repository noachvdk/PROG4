...
{
	auto SettingThread = std::thread([this, settings]() {this->m_LevelSettings.LoadLevelSettings(settings); });
...
bool LevelManager::GetIsDiscActiveByCoord(const glm::vec2& coord) const
{
	if (m_Disc01 && m_Disc01->GetCoord() == coord)
		return m_Disc01->GetIsActive();

	if (m_Disc02 && m_Disc02->GetCoord() == coord)
		return m_Disc02->GetIsActive();
...
void LevelManager::SetDiscActivated(const glm::vec2& coord) const
{
	if (m_Disc01 && m_Disc01->GetCoord() == coord)
		m_Disc01->SetActive();
